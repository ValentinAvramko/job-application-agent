...
class ExportResumePdfWorkflowTests(unittest.TestCase):
    def test_workflow_writes_pdf_and_runtime_verification_artifacts(self) -> None:
        ...
        with patch("application_agent.export_resume_pdf.generate_pdf_previews", side_effect=fake_preview_renderer):
            result = ExportResumePdfWorkflow().run(
                layout=layout,
                store=store,
        ...
        self.assertEqual(result.workflow, "export-resume-pdf")
        self.assertEqual(result.status, "completed")
        self.assertIn("CTO", result.summary)
        self.assertTrue(pdf_path.exists())
        self.assertTrue(report_path.exists())
        self.assertTrue(preview_path.exists())
        self.assertIn("ru/EU", result.summary)
        self.assertEqual(
            snapshot["task_memory"]["active_artifacts"],
            [str(pdf_path), str(report_path), str(preview_path)],
        )
        self.assertEqual(snapshot["task_memory"]["active_workflow"], "export-resume-pdf")
        self.assertIsNone(snapshot["task_memory"]["active_vacancy_id"])
        self.assertEqual(snapshot["workflow_runs"][-1]["workflow"], "export-resume-pdf")
    ...
    def test_workflow_rejects_unknown_resume(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "Unknown target_resume"):
            ExportResumePdfWorkflow().run(
                layout=layout,
                store=store,
                request=ExportResumePdfRequest(target_resume="VP"),
            )
