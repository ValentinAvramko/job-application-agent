...
class TestPrepareScreeningWorkflow:
    ...
    def test_prepare_screening_creates_artifact_and_updates_memory(self) -> None:
        ...
        with patch('application_agent.workflows.ingest_vacancy.validate_response_monitoring_workbook', return_value=None), patch('application_agent.workflows.ingest_vacancy.append_ingest_record', return_value=7):
        ...
        assert '## Паспорт' in screening_text
        assert '## Мини-сценарий самопрезентации' in screening_text
        assert '## Сценарий разговора' in screening_text
        assert '## Вероятные вопросы на screening' in screening_text
        assert 'Выбранное резюме: HoD' in screening_text
        assert 'status: screening_prepared' in meta_text
        assert 'selected_resume: HoD' in meta_text
        assert task_memory['active_workflow'] == 'prepare-screening'
        assert len(workflow_runs) == 3
        assert workflow_runs[-1]['workflow'] == 'prepare-screening'
    ...
    def test_prepare_screening_works_with_placeholder_analysis(self) -> None:
        ...
        vacancy_id = store.load_task_memory().active_vacancy_id
        assert vacancy_id is not None
        result = PrepareScreeningWorkflow().run(layout=layout, store=store, request=PrepareScreeningRequest(vacancy_id=vacancy_id or '', preparation_depth='deep'))
        screening_text = (layout.vacancy_dir(vacancy_id or '') / 'screening.md').read_text(encoding='utf-8')
        assert result.status == 'completed'
        assert 'Выбранное резюме: HoD' in screening_text
        assert 'Глубина подготовки: deep' in screening_text
        assert 'Что стоит подсветить' in screening_text
        assert 'Что спросить в ответ' in screening_text
    ...
    def test_prepare_screening_reports_incomplete_vacancy(self) -> None:
        ...
        with pytest.raises(FileNotFoundError, match='Runtime memory or the provided vacancy_id is stale'):
            PrepareScreeningWorkflow().run(layout=layout, store=store, request=PrepareScreeningRequest(vacancy_id='20260421-missing-role'))
