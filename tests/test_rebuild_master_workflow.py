...
class TestRebuildMasterWorkflow:
    ...
    def test_workflow_updates_master_and_runtime_report_without_touching_role_resumes(self) -> None:
        ...
        write_accepted_signals(accepted_path, [AcceptedSignal(signal='Leadership of 6 teams through engineering leads', target='MASTER.md', source_vacancy='vacancy-1', rationale='Approved durable leadership signal.', updated_at='2026-04-22T18:40:00+00:00')])
        ...
        assert result.workflow == 'rebuild-master'
    ...
    def test_workflow_no_op_when_master_already_matches_current_signals(self) -> None:
        ...
        accepted_path = layout.adoptions_dir / 'accepted' / 'MASTER.md'
        write_accepted_signals(accepted_path, [AcceptedSignal(signal='OpenAI and Codex as working AI tools', target='MASTER.md', source_vacancy='vacancy-2', rationale='Confirmed as reusable AI tooling signal.', updated_at='2026-04-22T18:41:00+00:00')])
        ...
        assert 'added=1' in first.summary
        assert 'already matches' in second.summary
...
def write_resume(path: Path) -> None:
    ...
    path.write_text('\n'.join(['# Candidate', '', '## О себе', '', '- Existing profile summary.', '', '## Рекомендации', '', 'References available on request.', '']), encoding='utf-8', newline='\n')
