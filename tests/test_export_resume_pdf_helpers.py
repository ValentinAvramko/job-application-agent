...
from application_agent.export_resume_pdf import (  # noqa: E402
    ExportResumePdfDependencyError,
    apply_export_resume_pdf_projection,
    generate_pdf_previews,
)
from application_agent.workspace import WorkspaceLayout  # noqa: E402
...
class ExportResumePdfHelpersTests(unittest.TestCase):
    def test_helper_renders_pdf_report_and_is_idempotent(self) -> None:
        ...
        with patch("application_agent.export_resume_pdf.generate_pdf_previews", side_effect=fake_preview_renderer) as preview_mock:
            first = apply_export_resume_pdf_projection(
                target_resume="CTO",
                output_language="ru",
                contact_region="EU",
                template_id="default",
            ...
            second = apply_export_resume_pdf_projection(
                target_resume="CTO",
                output_language="ru",
                contact_region="EU",
                template_id="default",
        ...
        self.assertTrue(pdf_output_path.exists())
        self.assertGreater(pdf_output_path.stat().st_size, 0)
        self.assertEqual(first.page_count, 1)
        self.assertEqual(second.page_count, 1)
        self.assertEqual(first.projection.surface.location, "Bilbao, Spain")
        self.assertIn("Телефон: +34 624 43 44 92", first.projection.surface.contacts)
        self.assertIn("LinkedIn: https://linkedin.com/in/Avramko", first.projection.surface.public_links)
        self.assertEqual(first.preview_files[0].name, "page-1.png")
        self.assertIn("- Target resume: CTO", report)
        self.assertIn("- Contact region: EU", report)
        self.assertIn("- Template: default", report)
        self.assertIn("- Preview pages: 1", report)
    ...
    def test_helper_rejects_invalid_inputs_before_rendering(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "supports only 'ru'"):
            apply_export_resume_pdf_projection(
                target_resume="MASTER",
                output_language="en",
                contact_region="RU",
                template_id="default",
                resume_path=resume_path,
        ...
        with self.assertRaisesRegex(ValueError, "supports only 'default'"):
            apply_export_resume_pdf_projection(
                target_resume="MASTER",
                output_language="ru",
                contact_region="RU",
        ...
        with self.assertRaisesRegex(ValueError, "Contact region 'US'"):
            apply_export_resume_pdf_projection(
                target_resume="MASTER",
                output_language="ru",
                contact_region="US",
    ...
    def test_helper_raises_preview_dependency_error_without_partial_writes(self) -> None:
        ...
        with patch("application_agent.export_resume_pdf.shutil.which", return_value=None):
            with self.assertRaisesRegex(ExportResumePdfDependencyError, "pdftoppm"):
                apply_export_resume_pdf_projection(
                    target_resume="MASTER",
                    output_language="ru",
                    contact_region="RU",
        ...
        self.assertFalse(pdf_output_path.exists())
    ...
    def test_generate_pdf_previews_uses_pdftoppm_and_collects_sorted_pages(self) -> None:
        ...
        self.assertEqual([path.name for path in previews], ["page-1.png", "page-2.png"])
    ...
    path.write_text(
        "\n".join(
            [
                ...
                '    phone: "+7 (918) 990-10-09"',
                '    email: "avramko@mail.ru"',
                '    telegram: "@ValentinAvramko"',
                '    whatsapp: "+7 (918) 990-10-09"',
                "  EU:",
                '    location: "Bilbao, Spain"',
                '    relocation: "Open to relocation"',
                '    phone: "+34 624 43 44 92"',
                '    email: "valentin.avramko@gmail.com"',
                '    telegram: "@ValentinAvramko"',
                '    whatsapp: "+34 624 43 44 92"',
                "links:",
                '  linkedin: "https://linkedin.com/in/Avramko"',
                '  github: "https://github.com/ValentinAvramko"',
                "",
            ]
        ),
        encoding="utf-8",
    ...
    path.write_text(
        "\n".join(
            [
                ...
                '  ru: "Валентин Аврамко"',
                '  eu: "Valentin Avramko"',
                "desired_roles: \"Технический директор / CTO\"",
                "location:",
                '  ru: "Краснодар, Россия"',
                '  eu: "Bilbao, Spain"',
                "contacts:",
                ...
                "## Ключевые достижения",
                "",
                "- Снизил Lead Time и стабилизировал поставку изменений.",
                "- Построил управляемый процесс разработки и релизов.",
                "",
                "## Опыт работы",
                "",
                "### Free2Trip",
                ...
                "**CTO**",
                "",
                "- Отвечал за архитектуру и delivery цифровой платформы.",
                "",
                "## Рекомендации",
                "",
                "Контакты могу предоставить по запросу.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    path.write_text(
        "\n".join(
            [
                ...
                "## Ключевые акценты",
                "",
                "- Platform engineering и delivery systems.",
                ...
                "**CTO**",
                "",
                "- Руководил архитектурой и поставкой продукта.",
                "",
                "## Рекомендации",
                "",
                "Контакты могу предоставить по запросу.",
