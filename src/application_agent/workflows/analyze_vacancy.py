...
ANALYZE_PROMPT_FILES = {
    ...
    "resume_selection_contract": "resume-selection.ru.md",
    "analysis_contract": "analysis-contract.ru.md",
    "cover_letter_contract": "cover-letter-contract.ru.md",
    "resume_adaptation_contract": "resume-adaptation-contract.ru.md",
    "humanizer_pass_prompt": "humanizer-pass.ru.md",
}
LLM_REQUEST_TIMEOUT_SECONDS = 600
HUMANIZER_REQUEST_TIMEOUT_SECONDS = 300
...
class AnalyzePromptBundle:
    system_prompt: str
    task_prompt: str
    resume_selection_contract: str
    analysis_contract: str
    cover_letter_contract: str
    resume_adaptation_contract: str
    humanizer_pass_prompt: str
...
class OpenAICompatibleProvider(LLMProvider):
    def generate(self, *, evidence_pack: dict[str, Any], config: LLMRuntimeConfig) -> dict[str, Any]:
                            ...
                            "text": json.dumps(
                                {
                                    ...
                                    "контракт_выбора_резюме": prompt_bundle.resume_selection_contract,
                                    "контракт_анализа": prompt_bundle.analysis_contract,
                                    "контракт_сопроводительного_письма": prompt_bundle.cover_letter_contract,
                                    "контракт_адаптации_резюме": prompt_bundle.resume_adaptation_contract,
        ...
        raw = self._request_json(payload=payload, config=config, timeout=LLM_REQUEST_TIMEOUT_SECONDS)
    ...
    def humanize_cover_letters(
        ...
        *,
            ...
            "input": [
                {
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": prompt_bundle.humanizer_pass_prompt,
                        }
                    ],
                },
                {
                    ...
                    "content": [
                        {
                            ...
                            "text": json.dumps(
                                {
                                    "humanizer_pass_task": prompt_bundle.humanizer_pass_prompt,
                                    ...
                                    "required_json_keys": ["cover_letter_standard", "cover_letter_short"],
                                },
                                ensure_ascii=False,
                            ),
                        }
        ...
        apply_llm_config(payload, config)
        raw = self._request_json(payload=payload, config=config, timeout=HUMANIZER_REQUEST_TIMEOUT_SECONDS)
        content = extract_response_output_text(raw)
        try:
...
def prompt_bundle_from_evidence_pack(evidence_pack: dict[str, Any]) -> AnalyzePromptBundle:
    ...
    return AnalyzePromptBundle(
        ...
        resume_selection_contract=str(raw.get("resume_selection_contract", "")).strip(),
        analysis_contract=str(raw.get("analysis_contract", "")).strip(),
        cover_letter_contract=str(raw.get("cover_letter_contract", "")).strip(),
        resume_adaptation_contract=str(raw.get("resume_adaptation_contract", "")).strip(),
        humanizer_pass_prompt=str(raw.get("humanizer_pass_prompt", "")).strip(),
...
def load_analyze_prompt_bundle(
    *,
    ...
    return AnalyzePromptBundle(
        ...
        resume_selection_contract=prompts["resume_selection_contract"],
        analysis_contract=prompts["analysis_contract"],
        cover_letter_contract=prompts["cover_letter_contract"],
        resume_adaptation_contract=prompts["resume_adaptation_contract"],
        humanizer_pass_prompt=prompts["humanizer_pass_prompt"],
